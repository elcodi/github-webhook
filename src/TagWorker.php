 ...
 * list is unique
 */
while ($poppedValue = $redis->blpop('github-tag-queue', 0)) {

    $commit = $poppedValue[1];
    $logger->addInfo("Starting tag process. HEAD commit is [$commit]");

    $process = (new ProcessBuilder([__DIR__ . '/tag.sh ' . $poppedValue]))->getProcess();
    ...
    if (!$process->isSuccessful()) {
        $logger->addError('Error tagging repo: ' . $process->getErrorOutput());
        continue;
    }

    $logger->addInfo("Done tagging repo.");
}
